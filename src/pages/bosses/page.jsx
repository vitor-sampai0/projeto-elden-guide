          ...
          {usuarios.map((boss) => {
            return (
              <div key={boss.id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow border border-gray-200">
                ...
                <p className="text-gray-600 mb-3 line-clamp-3">{boss.description}</p>
                <p className="text-sm text-yellow-600 font-medium mb-3">📍 Local: {boss.location}</p>
                <img src={boss.image} alt={boss.name} className="mt-2 w-full h-48 object-cover rounded-md" />
              </div>
            );
          })}
        </div>
